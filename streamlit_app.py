...
def get_genai_client():
    # Streamlit Cloud 會從 Secrets 中讀取 GEMINI_API_KEY
    # 本地運行則會抓取環境變數
    ...
    return genai.Client(
        ...
        http_options={'api_version': 'v1'} # 使用穩定版 v1 避免 404
    )
...
def get_db_collection():
    try:
        ...
        # get_or_create 避免第一次運行時因找不到 collection 而崩潰
...
with st.sidebar:
    ...
    st.info(f"📊 當前模型：`{GEN_MODEL_ID.split('/')[-1]}`")
    st.markdown("---")
    ...
    if not user_input:
        ...
        with st.spinner("🔍 正在搜尋案例庫並分析風險中..."):
            try:
                ...
                if "429" in str(e):
                    st.error("⚠️ 額度用盡 (Rate Limit)，請等待 30 秒後重試。")
                elif "404" in str(e):
                    st.error("❌ 模型路徑錯誤，請確認 API 版本與模型 ID。")
                else:
