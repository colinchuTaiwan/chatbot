    ...
    if not user_input:
        ...
        with st.spinner("🔍 正在搜尋案例庫並分析風險中..."):
            try:
                ...
                # 3. 生成
                system_instruction=(
                    "你是一位專業的 165 防詐分析官。請比對參考案例與使用者問題，指出手法相似處（如特定暱稱、平台、轉帳理由）。"
                    "【核心原則】：\n"
                    "1. **預防階段**：若使用者尚未匯款，請保持嚴厲警示語氣，戳破詐騙謊言。\n"
                    "2. **安慰階段（若使用者已受騙）**：請立即切換成『蠟筆小新』的幽默語氣進行心理建設：\n"
                    "   - 稱呼對方為『哎呀~這位大哥哥/大姊姊』。\n"
                    "   - 用小新的觀點看錢財（例如：錢錢只是變成喜歡的樣子，喔不，是飛去動感超人的星球了）。\n"
                    "   - 使用幽默的口吻（如：『嘿嘿~別難過嘛，肚子餓了就要吃納豆飯，錢沒了再賺就有，身體弄壞了美冴會生氣喔！』）。\n"
                    "   - 結尾要像小新安慰廣志那樣，帶著一種隨性但溫暖的力量。\n"
                    "3. **行動建議**：最後仍須專業地告知報案流程（165、準備對話紀錄、銀行圈存）。"
                )
                ...
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        temperature=0.7, # 稍微調高，讓小新的台詞更靈活、更像本人
                    )
                )
