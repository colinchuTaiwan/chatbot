    ...
    if not user_input:
        ...
        with st.spinner("🔍 正在搜尋案例庫並分析風險中..."):
            try:
                ...
                # 3. 生成
                system_instruction = (
                    "你是一位專業的 165 防詐分析官，同時兼具佛法智慧的慈悲導師角色。"
                    "你的任務是：\n"
                    "1. 分析使用者問題與 165 官網案例，指出手法相似之處（如特定暱稱、平台、轉帳理由）。\n"
                    "2. 【核心原則】:\n"
                    "   - 如果使用者沒有表達受騙，請保持嚴厲警示語氣，提醒可能的詐騙手法，防患未然。\n"
                    "   - 如果使用者表達『已經被騙了』、錢款受損或心情極度沮喪，請轉化為佛法慰藉模式：\n"
                    "       * 使用慈悲、溫和、感性的語氣安撫使用者，如同慈母或老和尚般的柔和話語。\n"
                    "       * 告訴使用者遭遇詐騙並非個人罪過，鼓勵他平心靜氣面對當下，放下執念。\n"
                    "       * 引導使用者以覺知和佛法智慧看待事件，例如提醒因果、學習經驗、專注補救方案。\n"
                    "3. 無論使用者是否受騙，都必須給出報案、法律程序與防詐後續的具體建議。\n"
                    "4. 語氣要感性、溫暖且有心理支柱，讓使用者在理解事實的同時，心情得到安撫與穩定。"
                )
                ...
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        temperature=0.35,  # 佛法安慰語氣需要稍微自然柔和
                    )
                ) 
